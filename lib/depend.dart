//library

export 'package:animations/animations.dart';
export 'dart:io';
export 'dart:convert';
export 'package:animations/animations.dart';
export 'package:auto_size_text/auto_size_text.dart';
export 'package:firebase_auth/firebase_auth.dart';
export 'package:flare_flutter/flare_actor.dart';
export 'package:flutter/material.dart';
export 'package:flutter/services.dart';
export 'package:sweetsheet/sweetsheet.dart';
export 'package:modal_progress_hud/modal_progress_hud.dart';
export 'package:pin_code_fields/pin_code_fields.dart';
export 'package:shared_preferences/shared_preferences.dart';
export 'package:cloud_firestore/cloud_firestore.dart';
export 'package:localstorage/localstorage.dart';
export 'package:shimmer/shimmer.dart';
export 'package:provider/provider.dart';
export 'package:flashy_tab_bar/flashy_tab_bar.dart';
export 'package:firebase_storage/firebase_storage.dart';
export 'package:gradient_app_bar/gradient_app_bar.dart';
export 'package:progress_dialog/progress_dialog.dart';
export 'package:image_cropper/image_cropper.dart';
export 'package:image_picker/image_picker.dart';
export 'package:modal_progress_hud/modal_progress_hud.dart';
export 'package:random_string/random_string.dart';
export 'package:fluttertoast/fluttertoast.dart';
export 'package:cached_network_image/cached_network_image.dart';
export 'dart:math' show Random;
export 'package:tutorial_coach_mark/animated_focus_light.dart';
export 'package:tutorial_coach_mark/tutorial_coach_mark.dart';
export 'package:flutter_calendar/flutter_calendar.dart';
export 'package:cloud_functions/cloud_functions.dart';
export 'package:auto_size_text/auto_size_text.dart';
export 'package:flutter_local_notifications/flutter_local_notifications.dart';
export 'package:google_maps_flutter/google_maps_flutter.dart';
export 'dart:collection';
export 'dart:typed_data';
export 'package:badges/badges.dart';
export 'package:line_awesome_flutter/line_awesome_flutter.dart';
export 'package:async/async.dart';
export 'dart:collection';
export 'package:geocoder/geocoder.dart';
export 'dart:async';
export 'package:location/location.dart';
export 'package:shake/shake.dart';
export 'package:vibration/vibration.dart';
export 'package:autostart/autostart.dart';
//export 'package:keyboard_avoider/keyboard_avoider.dart';
export 'package:full_screen_image/full_screen_image.dart';
export 'package:focused_menu/focused_menu.dart';
export 'package:path_provider/path_provider.dart';
//tabs





export 'package:ifela/tabs/contacts.dart';
export 'package:ifela/tabs/dashbord.dart';
export 'package:ifela/tabs/home.dart';
export 'package:ifela/tabs/settings.dart';
export 'package:ifela/tabs/chat.dart';




//screens

export 'package:ifela/screens/home.dart';
export 'package:ifela/screens/contacts.dart';
export 'package:ifela/screens/editprofile.dart';
export 'package:ifela/screens/tabs.dart';
export 'package:ifela/screens/updateprofile.dart';
export 'package:ifela/screens/authpage.dart';


//other Pages

export 'package:ifela/auth.dart';
export 'package:ifela/router.dart';
export 'package:ifela/utility.dart';
export 'package:ifela/constants.dart';

 

//Settings
export 'package:ifela/settings/fencing.dart';
export 'package:ifela/settings/pendingcontact.dart';
export 'package:ifela/auth.dart';